...
export function POSProvider({ children }) {
  ...
  useEffect(() => {
    async function fetchAll() {
      setLoading(true);
      await Promise.all([
        refetchTables(), refetchMenu(), refetchCategories(),
        refetchOrders(), refetchOrderItems(), refetchProfiles(),
        refetchIngredients(), refetchRecipeIngredients(),
      ]);
      setLoading(false);
    }
    fetchAll();
  }, [refetchTables, refetchMenu, refetchCategories, refetchOrders, refetchOrderItems, refetchProfiles, refetchIngredients, refetchRecipeIngredients]);
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'order_items' }, () => refetchOrderItems())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'restaurant_tables' }, () => refetchTables())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'menu_items' }, () => refetchMenu())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'ingredients' }, () => refetchIngredients())
      .subscribe();
    return () => { channel.unsubscribe(); };
  }, [refetchOrders, refetchOrderItems, refetchTables, refetchMenu, refetchIngredients]);

  // ---- Inventory: deduct stock ----
  const deductIngredients = useCallback(async (cartItems) => {
    // For each cart item, find recipe_ingredients and deduct stock
    for (const item of cartItems) {
      const menuItemId = item.id || item.menu_item_id;
      if (!menuItemId) continue;
      const recipes = recipeIngredients.filter((ri) => ri.menu_item_id === menuItemId);
      for (const recipe of recipes) {
        ...
        if (ing) {
          ...
          await supabase.from('ingredients').update({ stock: newStock, updated_at: new Date().toISOString() }).eq('id', recipe.ingredient_id);
        }
      }
    }
  ...
  // ---- Table management ----
  const addTable = useCallback(async (tableNumber, capacity = 4) => {
  ...
  const updateIngredient = useCallback(async (id, updates) => {
    await supabase.from('ingredients').update({ ...updates, updated_at: new Date().toISOString() }).eq('id', id);
    await refetchIngredients();
  }, [refetchIngredients]);
  ...
  const addRecipeIngredient = useCallback(async (menuItemId, ingredientId, quantityNeeded) => {
  ...
  const removeRecipeIngredient = useCallback(async (id) => {
    await supabase.from('recipe_ingredients').delete().eq('id', id);
    await refetchRecipeIngredients();
  }, [refetchRecipeIngredients]);

  // ---- Order operations ----
  const createOrder = useCallback(async (tableNumber, serverName, items = [], orderType = 'DINE-IN') => {
    ...
    if (tableNumber) {
      await supabase.from('restaurant_tables')
        .update({ status: 'OCCUPIED', current_bill: total, occupied_since: new Date().toISOString() })
  ...
  const addItemsToOrder = useCallback(async (orderId, items) => {
    ...
    if (allItems) {
      const subtotal = allItems.reduce((sum, oi) => sum + Number(oi.price) * oi.quantity, 0);
      const tax = parseFloat((subtotal * TAX_RATE).toFixed(2));
      const total = parseFloat((subtotal + tax).toFixed(2));
      ...
      if (order && order.table_number) {
        await supabase.from('restaurant_tables').update({ current_bill: total }).eq('table_number', order.table_number);
      }
    }

    await Promise.all([refetchOrders(), refetchOrderItems(), refetchTables()]);
  }, [refetchOrders, refetchOrderItems, refetchTables, deductIngredients, orders]);

  const removeOrderItem = useCallback(async (orderItemId, orderId) => {
    await supabase.from('order_items').delete().eq('id', orderItemId);

    const { data: remaining } = await supabase.from('order_items').select('*').eq('order_id', orderId);
    if (remaining) {
      const subtotal = remaining.reduce((sum, oi) => sum + Number(oi.price) * oi.quantity, 0);
      const tax = parseFloat((subtotal * TAX_RATE).toFixed(2));
      const total = parseFloat((subtotal + tax).toFixed(2));
      await supabase.from('orders').update({ subtotal, tax, total }).eq('id', orderId);
    }
  ...
  const updateOrderItemStatus = useCallback(async (orderItemId, status) => {
    await supabase.from('order_items').update({ status }).eq('id', orderItemId);
    await refetchOrderItems();
  }, [refetchOrderItems]);

  const billOutTable = useCallback(async (tableNumber) => {
    // Get current orders for receipt generation before completing them
    const tableOrders = orders.filter((o) => o.table_number === tableNumber && o.status !== 'COMPLETED' && o.status !== 'CANCELLED');
    const tableItems = tableOrders.flatMap((o) => orderItems.filter((oi) => oi.order_id === o.id));

    await supabase.from('orders').update({ status: 'COMPLETED', updated_at: new Date().toISOString() }).eq('table_number', tableNumber).neq('status', 'COMPLETED');
    await supabase.from('restaurant_tables')
      .update({ status: 'EMPTY', current_bill: 0, guests_count: 0, occupied_since: null })
      .eq('table_number', tableNumber);
  ...
  const getOrdersForTable = useCallback(
    (tableNumber) => orders.filter((o) => o.table_number === tableNumber && o.status !== 'COMPLETED' && o.status !== 'CANCELLED'),
    [orders]
  );
  ...
  const getRecipeForItem = useCallback(
    (menuItemId) => recipeIngredients.filter((ri) => ri.menu_item_id === menuItemId),
    [recipeIngredients]
  );

  const getLowStockIngredients = useCallback(
    () => ingredients.filter((i) => Number(i.stock) <= Number(i.low_stock_threshold)),
    [ingredients]
  );

  const calculateBill = useCallback((items) => {
    const subtotal = items.reduce((sum, oi) => sum + Number(oi.price) * oi.quantity, 0);
    const tax = parseFloat((subtotal * TAX_RATE).toFixed(2));
    const total = parseFloat((subtotal + tax).toFixed(2));
  ...
  return (
    <POSContext.Provider value={{
      tables, menuItems, categories, orders, orderItems, profiles,
      ingredients, recipeIngredients,
      loading, CURRENCY, TAX_RATE,
      addTable, removeTable,
      addMenuItem, updateMenuItem, deleteMenuItem, addCategory,
      addIngredient, updateIngredient, deleteIngredient,
      addRecipeIngredient, removeRecipeIngredient,
      createOrder, addItemsToOrder, removeOrderItem,
      updateOrderStatus, updateOrderItemStatus, billOutTable,
      addProfile,
      getOrdersForTable, getItemsForOrder, getActiveOrders,
      getRecipeForItem, getLowStockIngredients,
      calculateBill, formatPrice,
      refetchOrders, refetchOrderItems, refetchTables, refetchIngredients,
    }}>
      {children}
    </POSContext.Provider>
  );
